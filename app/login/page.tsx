...
function LoginContent() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Authenticate directly against the encrypted database backend
      ...
      setErrorMessage(
        language === "EN"
          ...
          : "Ungültige E-Mail-Anmeldedaten oder falsches Kontopasswort."
      );
    } finally {
      setLoading(false);
    }
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="w-full max-w-md bg-white p-8 rounded-[2.5rem] border border-black/5 shadow-xl relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-[#1F1F1F]/50">
            {language === "EN" 
              ? "Log in to view live order tracking and saved details" 
              : "Logge dich ein, um dein Live-Tracking und deine Profildaten zu sehen"}
          </p>
        </div>

        {/* Security Input Feedback Triggers */}
        {errorMessage && (
        ...
        <form onSubmit={handleEmailLogin} className="space-y-4">
          ...
          >
            {loading ? (
              ...
              <>
                <span className="text-white">{language === "EN" ? "Secure Account Entry" : "Sicher Einloggen"}</span>
        ...
        <div className="relative my-6 text-center">
          ...
          <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-3 text-[10px] font-bold uppercase tracking-widest text-[#1F1F1F]/30">
            {language === "EN" ? "Or secure option" : "Oder sichere Option"}
          </span>
        </div>
        ...
        {/* Navigational Switch Anchor */}
        <div className="mt-6 text-center text-xs font-semibold">
          <span className="text-[#1F1F1F]/40">
            {language === "EN" ? "Don't have an account yet?" : "Noch kein Profil erstellt?"}{" "}
          </span>
          <Link href="/signup" className="text-[#CDAF95] hover:text-[#C9A24D] underline transition-colors duration-300">
...
// Global compilation boundary to capture router parameters safely
